...
export default async function ProjectPage({ params }: ProjectPageProps) {
  ...
  if (!user) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[calc(100vh-theme(spacing.16))] text-center">
        <h2 className="text-2xl font-bold">Please log in to view project details.</h2>
        <p className="text-muted-foreground mt-2">You can set up authentication in your Supabase project.</p>
      </div>
    )
  }
  ...
  const userRole = projectData.project_members.find((member) => member.profile_id === user.id)?.role

  if (!userRole) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[calc(100vh-theme(spacing.16))] text-center">
        <h2 className="text-2xl font-bold">Access Denied</h2>
        <p className="text-muted-foreground mt-2">You are not a member of this project.</p>
      </div>
    )
  }
  ...
  const completedTasks = projectData.tasks.filter((task) => task.status === "done").length
  ...
  const toggleTaskStatus = async (taskId: string, newStatus: string) => {
    ...
    if (memberError || !member || (member.role !== "creator" && member.role !== "admin")) {
      throw new Error("Permission denied. Only creators/admins can update tasks.")
    }
  ...
  const deleteTask = async (taskId: string) => {
    ...
    if (memberError || !member || (member.role !== "creator" && member.role !== "admin")) {
      throw new Error("Permission denied. Only creators/admins can delete tasks.")
    }
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        ...
        <Card className="lg:col-span-2">
          <CardHeader>
            ...
            <CardDescription>Current status and progress of the project.</CardDescription>
          </CardHeader>
          <CardContent className="grid gap-4">
            <div>
              <div className="text-sm font-medium mb-2">Overall Progress</div>
              <Progress value={projectProgress} className="h-3" />
              <div className="text-xs text-muted-foreground mt-1">{projectProgress}% Completed</div>
            </div>
            <Separator />
              ...
              <div>
                ...
                {projectData.goals.length > 0 ? (
                  projectData.goals.map((goal) => (
                    <div key={goal.id} className="flex items-center justify-between text-sm py-1">
        ...
        <Card className="lg:col-span-3">
          <CardHeader>
            ...
            <CardDescription>Client and creator feedback on the project.</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="space-y-4 mb-6 max-h-96 overflow-y-auto pr-2">
              {projectData.feedback.length > 0 ? (
                projectData.feedback.map((feedback) => <FeedbackItem key={feedback.id} feedback={feedback} />)
              ) : (
                <p className="text-muted-foreground text-sm">No feedback yet. Be the first to leave one!</p>
              )}
            </div>
            <form action={addFeedback} className="flex flex-col gap-3">
              <Textarea name="feedbackContent" placeholder="Leave your feedback here..." rows={3} required />
