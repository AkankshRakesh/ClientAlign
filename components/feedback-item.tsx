...
export function FeedbackItem({ feedback }: FeedbackItemProps) {
  const senderName = feedback.profiles?.full_name || feedback.profiles?.email || "Unknown User"
    ...
    <Card className="mb-4">
      <CardHeader className="flex flex-row items-center justify-between pb-2">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm text-muted-foreground">{format(createdAt, "MMM dd, yyyy HH:mm")}</span>
        </div>
